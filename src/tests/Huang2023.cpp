...
namespace dagSched
{
    ...
    void fuzzyengineEstablish(fl::Engine &engine,float max_WCET,float MaxoutdegreeSum,float min_WCET,float MinoutdegreeSum,int MaxWaitTime)
    {
    MaxWaitTime=0;
    ...
    input1->setRange(min_WCET, max_WCET); // 设置输入范围
    input1->addTerm(new fl::ZShape("LOW", min_WCET,(max_WCET+min_WCET)/2.0)); // 添加隶属度函数
    // input1->addTerm(new fl::ZShape("LOW", 0,0.5));// // 添加隶属度函数
    // input1->addTerm(new fl::Gaussian("MEDIUM",0.5,0.125));
    input1->addTerm(new fl::Gaussian("MEDIUM",(max_WCET+min_WCET)/2.0,(max_WCET+min_WCET)/8.0));
    input1->addTerm(new fl::SShape("HIGH", (max_WCET+min_WCET)/2.0,max_WCET));
    ...
    input2->setRange(MinoutdegreeSum, MaxoutdegreeSum); // 设置输入范围
    input2->addTerm(new fl::ZShape("LOW", MinoutdegreeSum,(MaxoutdegreeSum+MinoutdegreeSum)/2.0)); // 添加隶属度函数
    input2->addTerm(new fl::Gaussian("MEDIUM",(MaxoutdegreeSum+MinoutdegreeSum)/2.0,(MaxoutdegreeSum+MinoutdegreeSum)/8.0));
    input2->addTerm(new fl::SShape("HIGH", (MaxoutdegreeSum+MinoutdegreeSum)/2.0,MaxoutdegreeSum));
    ...
    // input3->setName("waitTime");
    // input3->setRange(0.000, MaxWaitTime); // 设置输入范围
    // input3->addTerm(new fl::ZShape("LOW", 0,MaxWaitTime/2.0)); // 添加隶属度函数
    // input3->addTerm(new fl::Gaussian("MEDIUM",MaxWaitTime/2.0,MaxWaitTime/4.0));
    // input3->addTerm(new fl::SShape("HIGH", MaxWaitTime/2.0,MaxWaitTime));
    ...
    // ruleblock1->addRule(fl::Rule::parse("if waitTime is HIGH then output is HIGH with 0.2", &engine));
    // ruleblock1->addRule(fl::Rule::parse("if waitTime is MEDIUM then output is MEDIUM with 0.2", &engine));
    // ruleblock1->addRule(fl::Rule::parse("if waitTime is LOW then output is LOW with 0.2", &engine));
    ...
    float FuzzyCalculate1(float WCET,float outdegreeSum,int waitTime,fl::Engine &engine)
{
    // 计算模糊控制器的输出变量
    engine.setInputValue("WCET",WCET);
    engine.setInputValue("outdegreeSum",outdegreeSum);
    // engine.setInputValue("waitTime",waitTime);
    engine.process();
    ...
    std::vector<SubTask *> PriOrder(std::vector<SubTask *> &V,std::vector<float> WCET,std::vector<int> outdegreeSum,int coreNum,int pri,fl::Engine &engine,int mode)
    {
    ...
    while(V.size()!=assignedTask.size())
    {
        ...
        for(auto wait:WaitTask)
        {
            ...
            // {
                 wait->fpriValue=FuzzyCalculate1(wait->wcetSum,outdegreeSum[wait->id],wait->waitTime,engine);
            // }
    ...
    bool GP_FP_Huang2023(Taskset taskset, const int m)//!不抢占的EDF算法
    {
        //* 输入：任务集,核心数
        ...
        for (int i = 0; i < taskset.tasks.size(); ++i)
        {
            ...
            // timer.toc();
            auto maxit = std::max_element(WCET.begin(), WCET.end()); // 获取最大值的迭代器
            float max_WCET = *maxit; // 通过迭代器获取最大值

            auto minit = std::min_element(WCET.begin(), WCET.end()); // 获取最大值的迭代器
            // float min_WCET = *minit; // 通过迭代器获取最大值
            float min_WCET = *minit; // 通过迭代器获取最大值
            
            auto maxit1 = std::max_element(outdegreeSum.begin(), outdegreeSum.end()); // 获取最大值的迭代器
            int MaxoutdegreeSum = *maxit1; // 通过迭代器获取最大值

            auto minit1 = std::min_element(outdegreeSum.begin(), outdegreeSum.end()); // 获取最大值的迭代器
            int MinoutdegreeSum = *minit1; // 通过迭代器获取最大值

            int MaxWaitTime=V.size()-1;

            fuzzyengineEstablish(engine,max_WCET,MaxoutdegreeSum,min_WCET,MinoutdegreeSum,MaxWaitTime);
            //*进行优先级排序
            // timer.tic();
            
            std::vector<SubTask *> assignedTask;
            assignedTask=PriOrder(V,WCET,outdegreeSum,m,0,engine,0);
            AllassignedTask.push_back(assignedTask);
            // std::cout << "3:"<<timer.toc() << std::endl;
            // sleep(1);
            ...
            exectime+=Exec(assignedTask,m);
            // sleep(1);
            // std::cout << "subtask:" <<i << std::endl;
            if(exectime > taskset.tasks[i].getDeadline()) //*有一个超时了(这个可以放到Exec里面去 这样超了就不跑了)
             {
                // std::cout <<taskset.tasks[i].getDeadline() <<std::endl;
    ...
    int GP_FP_Huang2023_Multi(Taskset taskset, const int m)
    {
        ...
        for (int i = 0; i < taskset.tasks.size(); ++i)
        {
            ...
            // std::cout <<"Test!!!:" <<  V[0]->localD  << std::endl;
            auto maxit = std::max_element(WCET.begin(), WCET.end()); // 获取最大值的迭代器
            float max_WCET = V[0]->wcetSum; // 通过迭代器获取最大值
            // float max_WCET = *maxit; // 通过迭代器获取最大值
            auto minit = std::min_element(WCET.begin(), WCET.end()); // 获取最大值的迭代器
            float min_WCET = *minit; // 通过迭代器获取最大值
            
            // std::cout << "max:" << max_WCET << std::endl;
            // std::cout << "min:" << min_WCET << std::endl;
            auto maxit1 = std::max_element(outdegreeSum.begin(), outdegreeSum.end()); // 获取最大值的迭代器
            int MaxoutdegreeSum = *maxit1; // 通过迭代器获取最大值
            ...
            AllassignedTask.push_back(assignedTask);
        // std::cout << "Task" <<i <<":";
        // for(int k=0;k<assignedTask.size();k++)
        // {
        //     std::cout <<  assignedTask[k]->id << " " ;
        // }
        // std::cout << std::endl;
            //*得到每个任务的等待时间
            //*并行度
        ...
        //     {
        //         std::cout <<"Huang2023 suc response Time=" << responsetime[i] <<" Deadline:" <<taskset.tasks[i].getDeadline() << std::endl;
        //         schdulednumber++;
        //     }
        //     else
        //     {
        //         //  std::cout <<"response Time=" << responsetime[i] <<" Deadline:" <<taskset.tasks[i].getDeadline() << std::endl;
        //          std::cout <<"Huang2023 Fail response Time=" << responsetime[i] <<" Deadline:" <<taskset.tasks[i].getDeadline() << std::endl;
        //     }
        //  }
        ...
        for(int i=0;i<responsetime.size();i++)
        {
            if(responsetime[i]>taskset.tasks[i].getDeadline())
            {
                ...
                std::cout <<"response Time=" << responsetime[j] <<" Deadline:" <<taskset.tasks[j].getDeadline() << std::endl;
                 std::cout <<"Huang2023 fail response Time=" << responsetime[j] <<" Deadline:" <<taskset.tasks[j].getDeadline() << std::endl;
                }
